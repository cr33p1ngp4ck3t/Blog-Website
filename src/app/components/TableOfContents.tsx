...
export default function TableOfContents({ body }: TableOfContentsProps) {
  ...
  return (
    <div className="sticky top-24 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
      <h2 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white">Table of Contents</h2>
      <Accordion type="single" collapsible className="w-full">
        <AccordionItem value="item-1">
          <AccordionTrigger className="text-gray-900 dark:text-white">On this page</AccordionTrigger>
          <AccordionContent>
            <ul className="space-y-2">
              {headings.map((heading: TocEntry) => (
                <li key={heading._key} className={heading.level === 2 ? "pl-4" : heading.level === 3 ? "pl-8" : ""}>
                  <a
